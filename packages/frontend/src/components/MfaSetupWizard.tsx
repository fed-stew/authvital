...
interface MfaSetupWizardProps {
  ...
  onComplete: () => void;
  /** Optional banner rendered above the header (e.g. tenant policy notice). */
  banner?: ReactNode;
  /** Optional footer rendered below the scan/verify steps (e.g. skip button). */
  footer?: ReactNode;
}

/**
 * Shared MFA enrollment wizard (QR scan -> verify TOTP -> backup codes).
 *
 * Talks to the existing endpoints (POST /api/auth/mfa/setup and
 * POST /api/auth/mfa/enable) using the idp_session cookie. Used by both
 * /auth/mfa/setup and the /auth/mfa/enroll interrupt page - renders the card
 * CONTENT only; pages own the full-screen shell around it.
 */
export function MfaSetupWizard({ onComplete, banner, footer }: MfaSetupWizardProps) {
  ...
  return (
    <>
      ...
      {footer && (step === 'scan' || step === 'verify') && footer}
    </>
  );
}
