...
export class LicenseAdminController {
  ...
  async getApplicationSubscriptions(
    @Param('applicationId') applicationId: string,
  ) {
    return this.licensePoolService.getApplicationSubscriptions(applicationId);
  }
  ...
  async grantLicensesBulk(@Body() dto: BulkGrantLicenseDto) {
    return this.licenseAssignmentBulkService.grantLicensesBulk(dto.assignments);
  }

  @Post('licenses/revoke-bulk')
  async revokeLicensesBulk(@Body() dto: BulkRevokeLicenseDto) {
    return this.licenseAssignmentBulkService.revokeLicensesBulk(dto.revocations);
  }
